...
int SlideShow(void* p) {
	GUI* ui = static_cast<GUI*>(p);
	if (ui->_stop_slide_show) {
		std::cout << "slide show quit\n";
		return 0;
	}

	if (ui->article_loaded_) {
		std::cout << "read next article\n";
		ui->do_read_next_article(true);
	} else {
		std::cout << "wait for current article\n";
	}
	return 1;
}

void GUI :: do_read_next_article (bool from_slide_show)
{
	std::cout << "read next article\n";
	article_loaded_ = false;
	_header_pane->read_next_article ();

	if (!from_slide_show) {
		_stop_slide_show = false;
		g_timeout_add (1000, SlideShow, this);
	}
}

void GUI :: do_read_previous_article ()
{
	_stop_slide_show = true;
}
